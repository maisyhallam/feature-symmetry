...
with open('age_distinction_pruned.csv', 'a') as csv_file:
    csvwriter = csv.writer(csv_file)
    # csvwriter.writerow(['LANGUAGE','GENDER SYMMETRY','AGE SYMMETRY','LINEAGE SYMMETRY'])
    csvwriter.writerow(['LANGUAGE','AGE'])
...
for family in families:
    ...
    for file in files:
        ...
        language = get_language_name(file)
        # symmetries = []
        feature_symmetry_results = check_feature_symmetry(kin_terms,'age')
        gen_results = generation_results(feature_symmetry_results)

        write_full_csv('age_distinction_pruned.csv',file,gen_results)
