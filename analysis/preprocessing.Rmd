...
sim_z_scores <- sims %>%
  reframe(
    ...
    simulated_sd = sd(mutual_information)) %>%
  mutate(
    simulated_zscore = ifelse(simulated_value == simulated_mean,0, (simulated_value - simulated_mean) / simulated_sd),
    true_zscore = ifelse(true_value == simulated_mean, 0, (true_value - simulated_mean) / simulated_sd))
