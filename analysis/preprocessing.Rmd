...
```{r}
z_scores <- all %>%
  group_by(language,real_value,language_family) %>%
  summarise(
    simulated_mean = mean(simulated_value),
    simulated_sd = sd(simulated_value)) %>%
  mutate(
    zscore = (real_value - simulated_mean) / simulated_sd)
