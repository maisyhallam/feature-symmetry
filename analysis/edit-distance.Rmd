...
Still weird to be getting so many low z values - this would indicate that languages actively avoid having similar labels, rather than just accidentally having e.g. 'sister' share more letters with 'aunt' than cousin by chance. But I have an idea. Perhaps low mutual information may have far lower than chance average edit distance between parents and children intentionally, to further distinguish the category. From a production perspective, a similar label e.g. pur and purza helps you uncover meaning but is susceptible to ambiguity in a noisy channel. If we are not interested in encoding a meaning similarity between these kin, is it not beneficial to make their labels extremely far apart from each other? 
-> More likely, this is just because the averages we're working with are incredibly tiny.
...
```{r}
filter(mi, mutual_information < 1)
```
